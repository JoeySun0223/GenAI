...
功能：
- 清空并重建输出目录 SVG_Segmented_fixed/
- 遍历 SVG_Segmented/ 的一级子目录（若没有子目录，则直接处理根目录）
- 对每个 SVG 执行：
  1) 颜色面积统计 + “三步法 + ΔE”判定（删除或保留颜色）
  2) 清理“极短描边路径”（通常渲染成小黑点）

输出：
- 处理后的 SVG 到 SVG_Segmented_fixed/<同名子目录>/
...
INPUT_ROOT = "SVG_Segmented"
OUTPUT_ROOT = "SVG_Segmented_fixed"
