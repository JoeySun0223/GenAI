...
"""
PNG转SVG转换器
- 批量处理 Seg_output_fixed 下所有子文件夹的PNG图片
- 转换为SVG并保存到 SVG_Segmented 目录
- 保持原有的目录结构
"""
...
def main():
    # 每次运行都清空输出目录
    if os.path.exists("SVG_Segmented"):
        print(f"[INFO] 清空输出目录: SVG_Segmented")
        shutil.rmtree("SVG_Segmented")
    
    os.makedirs("SVG_Segmented", exist_ok=True)
    ...
    print("开始PNG转SVG转换...")
    print(f"输入目录: Seg_output_fixed")
    print(f"输出目录: SVG_Segmented")
    
    process_directory("Seg_output_fixed", "SVG_Segmented", params)
    
    print(f"\n转换完成！结果保存在: SVG_Segmented")
